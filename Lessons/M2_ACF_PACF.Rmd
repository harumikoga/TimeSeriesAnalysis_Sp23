...
# the ".." will point to your parent folder set as current directory - move to a parent folder. use two dots when using Document Directry 
...
my_date <- my(my_date)  #function my from package lubridate. if months come first use my. if year comes first, use ym. 
head(my_date)
...
plot(inflow_data[,"HP1"]) # note that this do not generate a nice plot. default is a scatterplot 
...
hist(inflow_data[,"HP1"],breaks=30,col="gray")  #increase number of bars with breaks input. -> log normal distribution is left skewed. 
...
#blue dot line is significance level. inside the line it is not significant. 

#you can change the number of lags
acf(ts_inflow_data[,1],lag.max=50) 
pacf(ts_inflow_data[,1],lag.max=50) 

#capital Acf. Acf doesn't show the correlation at zero it's always 1. 
library(forecast)
Acf(ts_inflow_data[,1],lag.max=50)
Pacf(ts_inflow_data[,1],lag.max=50)
